...
async def attack(update: Update, context: CallbackContext):
    ...
    if not ip.replace('.', '').isdigit() or not port.isdigit() or not duration.isdigit():
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Invalid input. Ensure IP, port and duration are valid.*", parse_mode='Markdown')
        return
...
def main():
    if not TELEGRAM_BOT_TOKEN or TELEGRAM_BOT_TOKEN == 'YOUR_TOKEN_HERE':
        print("ERROR: Set TELEGRAM_BOT_TOKEN environment variable or edit the token in the script.")
        return
